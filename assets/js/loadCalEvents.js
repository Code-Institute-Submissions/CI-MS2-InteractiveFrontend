...
$(document).ready(function () {
    ...
    $.getJSON(path, { key: API_KEY, maxResults: 5, singleEvents: "TRUE", orderBy: "startTime", timeMin: isoTime }, function (data, status) {
        ...
        $.each(data.items, function (i, val) {
            ...
            $.getJSON(path + val.id, { key: API_KEY }, function (eventData) {
                ...
                $("#calendar-events").append("<span style='padding-bottom: 5px;'><a href="+eventData.htmlLink+" target='_blank' class ='event-link'><i class='far fa-calendar-check'></i>: " + eventData.description + "</a><br><i class='fas fa-clock'></i>:  " + dateString + " " + timeString+ "<br><a target='_blank' href="+GMapsURI+gMapsLink+" class='event-link'><i class='fas fa-map-marker'></i>: "+eventData.location + "<br><br></span>");
